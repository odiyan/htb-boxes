...
* OFFICIAL SOLUTION
  +ok, this involves some LDAP stuff we haven't really learned I guess+. turns out there's a
  section on this [[file:d:/code/htb/academy/modules/active-directory-enumeration/miscellaneous-misconfig.org::*Sniffing LDAP Credentials][here]] (Sniffing LDAP Credentials in the Miscellaneous Misconfig section of
  AD enumeration)
  inside the =edit configuration= option in the PWM application, we can see a button for editing
  the LDAP connection; from the solution pdf:
  #+begin_src 
